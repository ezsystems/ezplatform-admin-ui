(function(global, doc, eZ, React, ReactDOM, Translator, flatpickr) {
    let getUsersTimeout;
    ...
    const formTrashCheckboxes = [...doc.querySelectorAll('form[name="trash_item_restore"] input[type="checkbox"]')];
    ...
    const enableButtons = () => {
        const isEmptySelection = formTrashCheckboxes.every((el) => !el.checked);
        const isMissingParent = formTrashCheckboxes.some((el) => el.checked && parseInt(el.dataset.isParentInTrash, 10) === 1);
    ...
    updateTrashForm(formTrashCheckboxes);
    enableButtons();
    formTrashCheckboxes.forEach((checkbox) => checkbox.addEventListener('change', handleCheckboxChange, false));
})(window, window.document, window.eZ, window.React, window.ReactDOM, window.Translator, window.flatpickr);
